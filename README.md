...
The repository includes a common benchmark runner for Otsu thresholding (with
and without 3-D watershed), the 3-D U-Net, Cellpose, StarDist, and masks
generated by other tools. It runs each method on matching test volumes, stores
instance-labeled TIFF masks, measures runtime and per-mask sizes, and computes
semantic and object-level metrics against Ilastik-derived or other ground
truth.
...
See [the segmentation benchmark guide](docs/segmentation-benchmark.md) for
learned-model options, required data layout, metric definitions, precomputed
mask support, and the complete output structure.
